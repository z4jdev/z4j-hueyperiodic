...
If you believe you have found a security vulnerability in `z4j-hueyperiodic`,
...
This read-only adapter inspects Huey's private periodic-task registry and sends
mapped schedule metadata to the brain. Registry compatibility and ensuring the
adapter never mutates decorator-defined schedules are package-specific
security surfaces; transport, redaction, and authorization policy remain owned
by `z4j-core` and the brain.
